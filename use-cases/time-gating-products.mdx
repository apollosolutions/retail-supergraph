...
async function getProduct(productId: String, releaseDate: Date = new Date()): Product {
    return productAPI.getProductById(productId).then(p => {
        const productReleaseDate = p.releaseDate ? new Date(p.releaseDate) : new Date();
        return productReleaseDate.getTime() < releaseDate.getTime() ? null : product;
    });
}
```
